...
def get_weather():
    ...
    with open('/home/pi/robot109/data/user_info.txt','r') as inf:
        location = eval(inf.read())["address"][2:-1].split(" ")[0]
    ...
    if date == '오늘': 
        ...
        with open('/home/pi/robot109/data/weather.txt','w') as inf:
            inf.write(text_Temp + ". " + text_feel_temp)
...
def get_Lotto():
    ...
    lotto = list()
    for num in result:
        lotto.append(num.text+"번")
    with open('/home/pi/robot109/data/lotto.txt','w') as inf:
        inf.write(" ".join(lotto))
    return " ".join(lotto)
    #동행복권 크롤링
